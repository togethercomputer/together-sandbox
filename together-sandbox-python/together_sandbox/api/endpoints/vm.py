...
class VmClient(VmClientProtocol):
    ...
    async def vm_assign_tag_alias(
        ...
        response = await self._transport.request(
            "PUT", url, params=None, json=json_body, headers=None
        )
    ...
    async def vm_update_hibernation_timeout(
        ...
        json_body: VmUpdateHibernationTimeoutRequest2 | None = (
            DataclassSerializer.serialize(body)
        )
        ...
        match response.status_code:
            case 200:
                return structure_from_dict(
                    response.json(), VmUpdateHibernationTimeoutResponse2
                )
            case _:
                raise HTTPError(
                    response=response,
