...
class SandboxClient(SandboxClientProtocol):
    ...
    async def sandbox_list(
        ...
        params: dict[str, Any] = {
            **({"tags": DataclassSerializer.serialize(tags)} if tags is not None else {}),
            **({"order_by": DataclassSerializer.serialize(order_by)} if order_by is not None else {}),
            **({"direction": DataclassSerializer.serialize(direction)} if direction is not None else {}),
            **({"page_size": DataclassSerializer.serialize(page_size)} if page_size is not None else {}),
            **({"page": DataclassSerializer.serialize(page)} if page is not None else {}),
            **({"status": DataclassSerializer.serialize(status)} if status is not None else {}),
        }
