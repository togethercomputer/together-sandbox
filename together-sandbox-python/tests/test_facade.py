...
class TestSandboxesNamespaceCreate:
    ...
    async def test_create_with_required_only(self):
        ...
        with patch(
            ...
            new_callable=AsyncMock,
            return_value=make_api_response(200, _make_sandbox_model()),
        ) as mock_api:
            await ns.create(
                millicpu=1000,
...
class TestExecsResume:
    ...
    async def test_resume_raises_on_error(self):
        ...
        with patch(
            ...
            with pytest.raises(
                RuntimeError, match="Failed to execs.resume for id 'exec-1'"
            ):
                await Execs(mock_client).resume("exec-1")
...
class TestExecsSendStdinAndResize:
    ...
    async def test_send_stdin_raises_on_error(self):
        ...
        with patch(
            ...
            with pytest.raises(
                RuntimeError, match="Failed to execs.sendStdin for id 'exec-1'"
            ):
    ...
    async def test_resize_raises_on_error(self):
        ...
        with patch(
            ...
            with pytest.raises(
                RuntimeError, match="Failed to execs.resize for id 'exec-1'"
            ):
                await Execs(mock_client).resize("exec-1", cols=80, rows=24)
