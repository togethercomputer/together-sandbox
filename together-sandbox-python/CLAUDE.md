...
```
together_sandbox/
├── __init__.py            # Public re-exports — keep in sync with the canonical export set
├── _together_sandbox.py   # hand-written facade entry point
├── _sandboxes.py          # hand-written sandbox lifecycle facade
├── _sandbox.py            # hand-written Sandbox + files/execs/ports/dirs facades
├── _snapshots.py          # hand-written snapshot facade
├── types.py               # hand-written public facade types + boundary adapters
├── _streaming.py          # SSE streaming helpers — hand-written
├── _utils.py / _lifecycle.py / _configuration.py / errors.py / docker.py  # hand-written
├── _api/                  # Generated — management API client (openapi-python-client)
└── _sandbox_client/       # Generated — in-VM sandbox API client (openapi-python-client)
```

**Every module not under `_api/` or `_sandbox_client/` is hand-written.**
Everything inside `_api/` and `_sandbox_client/` is regenerated by
`bash generate.sh` (run from the repo root) and must not be modified directly.
The underscore prefix marks these as private — they are not part of the public
API.

### Public types are hand-written facades

Facade methods return hand-written types from `types.py` (dataclasses +
`Literal` unions), never the generated attrs models. Each generated→facade
conversion goes through a `_*_from_model` / `_*_from_dict` adapter in `types.py`
called at the boundary. See **[EXPORTED_TYPES.md](../EXPORTED_TYPES.md)** for
the policy and the canonical export set.
...
- Build minimal valid fixture `SandboxModel` mocks (see the helpers in
  `tests/test_utils.py`) when a test needs a generated model.
